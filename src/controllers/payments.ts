...
export async function purchaseChallenge(req: Request, res: Response) {
  try {
    ...
    if (couponCode) {
      ...
      if (coupon && coupon.active && coupon.usedCount < coupon.limitUses && new Date() <= new Date(coupon.expiryDate)) {
        ...
        if (discountAmount > rule.price) discountAmount = rule.price;
      }
    }
    ...
    if (gateway === "RAZORPAY") {
      ...
      const secretConfig = await prisma.systemConfig.findUnique({ where: { key: "RAZORPAY_KEY_SECRET" } });
      const idConfig = await prisma.systemConfig.findUnique({ where: { key: "RAZORPAY_KEY_ID" } });
      ...
      try {
        ...
        const order: any = await rzpResponse.json();
        const proxyConfig = await prisma.systemConfig.findUnique({ where: { key: "RAZORPAY_PROXY_URL" } });
    ...
    const oxapayKey = await prisma.systemConfig.findUnique({ where: { key: "OXAPAY_API_KEY" } });
    ...
    try {
      ...
      const oxaData: any = await new Promise((resolve, reject) => {
...
export async function handleOxapayCallback(req: Request, res: Response) {
  try {
...
export async function verifyRazorpay(req: any, res: Response) {
  try {
    ...
    const idConfig = await prisma.systemConfig.findUnique({
    ...
    const { userId, challengeRuleId, couponCode } = order.notes as any;
    ...
    if (couponCode) {
      ...
      if (coupon && coupon.active) {
        if (coupon.discountType === "PERCENT") discountAmount = rule.price * (coupon.discountValue / 100);
 ...
 */
async function activateTradingAccount(account: any) {
  ...
  // Send onboarding email notification
  await sendTemplateEmail(account.user.email, "CHALLENGE_PURCHASED", {
...
export async function validateCoupon(req: Request, res: Response) {
  try {
    ...
    const rule = await prisma.challengeRule.findUnique({ where: { id: challengeRuleId } });
    if (!rule) return res.status(404).json({ error: "Challenge not found." });
