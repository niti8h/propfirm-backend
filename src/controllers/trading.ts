...
import { checkHftSpamRule } from "../riskGuardian";

// Helper to determine asset class leverage
function getLeverageLimit(symbol: string, rule: any): number {
...
export async function openTrade(req: Request, res: Response) {
  try {
    ...
    const isSpamming = await checkHftSpamRule(accountId, normalizedSymbol, normalizedDir);
    if (isSpamming) {
...
export async function cancelPendingOrder(req: Request, res: Response) {
  try {
    const { tradeId } = req.body;
