 ...
 */
export function calculateAccountEquity(account: any, openTrades: any[]): { equity: number; unrealizedPnl: number } {
  let unrealizedPnl = 0;
 ...
 */
export async function checkHftSpamRule(accountId: string, symbol: string, direction: string): Promise<boolean> {
 ...
 */
export async function checkInverseHedging(userId: string): Promise<boolean> {
  ...
  for (const t1 of openTrades) {
    for (const t2 of openTrades) {
      if (t1.accountId !== t2.accountId && t1.symbol === t2.symbol) {
        if (
          (t1.direction === "BUY" && t2.direction === "SELL") ||
  ...
  for (const account of activeAccounts) {
    ...
    if (equity <= absoluteLossFloor) {
      await breachAccount(
        account,
        openTrades,
        `Equity ($${equity.toFixed(2)}) breached absolute max loss floor ($${absoluteLossFloor.toFixed(2)})`,
        "MAX_LOSS_BREACH"
      );
      continue;
    }
    ...
    if (equity <= dailyFloor) {
      await breachAccount(
        account,
        openTrades,
        `Equity ($${equity.toFixed(2)}) breached daily drawdown floor ($${dailyFloor.toFixed(2)})`,
        "DRAWDOWN_BREACH"
      );
      continue;
    }
    ...
    if (userInverseHedging) {
      await breachAccount(
        account,
        openTrades,
        `Detected inverse hedging across user accounts`,
        "MAX_LOSS_BREACH"
      );
      continue;
    }
 ...
 */
async function breachAccount(account: any, openTrades: any[], reason: string, templateKey: "DRAWDOWN_BREACH" | "MAX_LOSS_BREACH") {
  ...
  // Notify user
  await sendTemplateEmail(account.user.email, templateKey, {
 ...
 */
async function advanceAccountPhase(account: any) {
  ...
  await sendTemplateEmail(account.user.email, emailKey, {
    Name: account.user.fullName,
    AccountID: account.id,
    CurrentEquity: `$${account.equity.toFixed(2)}`,
  });
 ...
 */
export async function performDailySnapshot() {
  ...
  for (const account of accounts) {
    ...
    const yesterdaySnapshot = await prisma.dailySnapshot.findFirst({
